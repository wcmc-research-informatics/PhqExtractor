...
/**
 * Finds the numeric values covered by the input annotation types and then
 * passes them to an abstract method for validation.
 * <p>
 * Created by thomasginter on 9/1/15. updated by olga patterson on 6/14/2016
 */
public abstract class NumericValidatorAnnotator extends LeoBaseAnnotator {
	 ...
	 */
	protected Pattern numberPattern = Pattern.compile("\\d+\\s*(\\/\\d+)?");

	/**
	 * Remove the annotation if any of the values are invalid, the default. If
	 * false then remove only if all the values are invalid.
	 */
	...
	public void annotate(JCas aJCas) throws AnalysisEngineProcessException {
		for (String inputType : inputTypes) {
			try {
				Collection<Annotation> annotations = AnnotationLibrarian.getAllAnnotationsOfType(aJCas, inputType,
						false);
	...
	/**
	 * Check the string value and return true if the value meets requirements.
	 *
	 * @param value
	 *            String value to be validated
	 * @return true if string is in phq-9 value format
	 */
	protected abstract boolean checkValid(String value);
