...
public class NumericMeasureFilter extends LeoBaseAnnotator {
  ...
  public void annotate(JCas aJCas) throws AnalysisEngineProcessException {
    ...
    Collection<gov.va.vinci.ef.types.Measurement> terms = AnnotationLibrarian.getAllAnnotationsOfType(aJCas, gov.va.vinci.ef.types.Measurement.type, false);
    
    /*for (gov.va.vinci.ef.types.Measurement term : terms) {
    	System.out.println("Term: " + term.getCoveredText());
    }*/
    ...
    for (NumericValue value : values) {
    	...
    	// if value covered text is 9, then check if that is attached to PHQ term
    	
      try {
        Window window = (Window) windowService.buildNonWhitespaceTokenWindow(value);
        
        // System.out.println("Value: " + value.getCoveredText());
        // System.out.println("Window: " + window.getCoveredText());
  ...
  public boolean hasPrePatternMatch(Annotation a, Annotation window) {
    ...
    //check if first letter is 9 in value and the preceeding pre-window has PHQ-, remove the value.
    
    String firstChar = a.getCoveredText().trim().substring(0,1);
    if (firstChar.equalsIgnoreCase("9")){
    	return coveredText.matches("(?is)(phq\\s*-\\s*)");
    }
    
    // System.out.println("Pre-Window: " + coveredText);
    // System.out.println("Value: " + a.getCoveredText());
  ...
  public boolean hasPostPatternMatch(Annotation a, Annotation window) {
    ...
    // System.out.println("Post-Window: " + coveredText);
