 ...
 * Updated by prakash adekkanattu on 08/25/2016
 */
public class ExtractorPipeline implements PipelineInterface {
  ...
  public LeoAEDescriptor getPipeline() throws Exception {
    ...
    // Module 1: MeasurementRegex -- find mentions of phq
    pipeline.addDelegate(new RegexAnnotator()
        .setResource(RESOURCE_PATH + "phqMeasure.regex")
    ...
    // Module 2: WindowAnnotator -- make a context window of -4...+4 tokens around the measurement phrase
    ...
    pipeline.addDelegate(new RegexAnnotator()
        .setResource(RESOURCE_PATH + "phqAnatomy.regex")
        .setCaseSensitive(false)
        .setPatternFlags(Pattern.DOTALL)
    ...
    pipeline.addDelegate(new RegexAnnotator()
        .setResource(RESOURCE_PATH + "phqNumericValue.regex")
        .setCaseSensitive(false)
        .setPatternFlags(Pattern.DOTALL)
    ...
    pipeline.addDelegate(new ConceptLinkAnnotator()
        ...
        .setMaxDifference(0)
        .setMaxDistance(10)
        .setPatternFile(RESOURCE_PATH + "phqRange.regex")
        .setRemoveCovered(true)
    ...
    pipeline.addDelegate(new NumericMeasureFilter()
        .setRegexFilePath(RESOURCE_PATH + "phqInvalidNumeric.regex")
        .setName("NumericMeasureFilter")
        .addLeoTypeSystemDescription(types)
        .getDescriptor());

    // Module 9: PHQValidatorAnnotator -- Filter out all numeric values that do not meet valid range criteria
    pipeline.addDelegate(new PHQValidatorAnnotator()
        .setRemoveIfAnyInvalid(false)
        .setInputTypes(new String[]{"gov.va.vinci.ef.types.NumericValue"})
        .setName("PHQValidatorAnnotator")
        .addLeoTypeSystemDescription(types)
        .getDescriptor());

    // Module 10: MeasureToValueRelationAnnotator -- Create PHQ concept-value pair based on the relational patterns between measurement and value
    pipeline.addDelegate(new MatchMakerAnnotator()
        ...
        .setOutputType("gov.va.vinci.ef.types.PhqRelation")
    ...
    // Module 11: 
    pipeline.addDelegate(new RegexAnnotator()
            .setResource(RESOURCE_PATH + "phqQuestionScore.regex")
            .setCaseSensitive(false)
            .setPatternFlags(Pattern.DOTALL)
            .setOutputType("gov.va.vinci.ef.types.QuestionScore")
            .setName("QuestionScoreRegex")
            .addLeoTypeSystemDescription(types)
            .getDescriptor());
    
    
    // Module13: FlattenRelationAE -- Create a new type for ease of writing results to csv or database
  ...
  public LeoTypeSystemDescription getLeoTypeSystemDescription() {
    ...
    try {
      description.addType(TypeLibrarian.getCSITypeSystemDescription())
          ...
          .addType(regexFtsd)
          .addType("gov.va.vinci.ef.types.PhqRelation", "", linkParent)
         ...
         // .addType("gov.va.vinci.ef.types.NoEvidence", "", "uima.tcas.Annotation")
          ...
          .addType("gov.va.vinci.ef.types.QuestionScore", "", regexParent)
          .addType("gov.va.vinci.kttr.types.RefSt_PhqValue", "", "uima.tcas.Annotation")
